...
subjectTag = 'F';
dataDirectory = '/cnmann/Downloads/data_mgl';
load(fullfile(dataDirectory, 'F', [subjectTag '_RT.mat']));
...
laterErrFcn = @(fits) -sum(log(compute_likelihood(fits, data)));
...
% 1. Visual Inspection
figure;
scatter(data, model, 'o');
xlabel('F');
ylabel('Model Predictions');
title('Model Fit');

% 2. Goodness-of-Fit Statistics (R-squared)
r_squared = 1 - sum((data - model).^2) / sum((data - mean(data)).^2);
...
fprintf('Best-fitting Parameters: muR = %.4f, deltaS = %.4f\n', fits);
